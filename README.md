...
│                                                                 │
│  ⚙️ ag_core/ ─────────────────────── ← 核心引擎层              │
│  ├── __init__.py     包初始化 (v3.5.0)                          │
│  ├── middleware.py    5层洋葱中间件管线 (Archon-Gate)            │
│  ├── dispatcher.py    并发调度器 V3.1 (信号量 + payload 透传)   │
│  ├── event_logger.py  事件日志写入器 (JSON 安全 + 10MB 旋转)    │
│  ├── ag_indexer.py    代码图谱引擎 (AST/Regex 多语言解析)       │
│  ├── ag_evolve.py     社区自进化引擎 (5源抓取 + 内容去重)       │
│  ├── ag_learning.py   深度学习引擎 (README/源码/API 解析)       │
│  ├── ag_review_army.py Review Army v2.0 (AST+Regex 三路静态分析) │
│  ├── ag_post_review.py 对话窗口审查闸门 (5维审查/BAT专项)       │
│  └── repo_cache.py    仓库缓存                                  │
│                                                                 │
...
> ⚠️ **适用范围**: 中间件在 Python 编排模式（通过 `AsyncGlobalDispatcher` 分发的任务）下自动生效。在 IDE 对话窗口中，可通过 `ag_post_review.py` 命令行工具获得等效的审查能力。
...
### 6. Review Army v2.0 真实静态分析 (`ag_core/ag_review_army.py`)

三路并行静态分析审查军团，零外部依赖，基于 Python AST + Regex：
- **Testing Specialist**: AST 分析裸 except、过长函数(>50行)、嵌套过深(≥4层) 等代码质量问题
- **Security Specialist**: 正则扫描硬编码密钥、eval/exec 注入、SQL 拼接、pickle 反序列化等安全漏洞
- **Consistency Specialist**: AST 检测未使用的导入 + 正则扫描 print 调试遗留、pdb 断点、TODO 标记
- **置信度校准**: 基于 `_learnings.jsonl` 历史记录自动压低已知误报的告警权重
- **Fix-First 三分类**: `[AUTO-FIX]`（可自动修复）、`[ASK]`（需人工定夺）、`[INFO]`（优化建议）

```bash
# 命令行直接使用
python3 ag_core/ag_review_army.py path/to/file1.py path/to/file2.py
```
...
```bash
# 1. 克隆仓库
git clone git@github.com:caishen20108899-cloud/Antigravity-AI-behavior-governance-operating-system.git
cd Antigravity-AI-behavior-governance-operating-system
...
├── ag_core/                    # 核心引擎
│   ├── __init__.py             # 包初始化
│   ├── middleware.py           # 5层洋葱中间件
│   ├── dispatcher.py           # 并发调度器
│   ├── event_logger.py         # 事件日志(安全)
│   ├── ag_indexer.py           # 代码图谱引擎
│   ├── ag_evolve.py            # 社区自进化引擎
│   ├── ag_learning.py          # 深度学习引擎
│   ├── ag_review_army.py       # Review Army v2.0 静态分析
│   ├── ag_post_review.py       # 对话窗口审查闸门
│   └── repo_cache.py           # 仓库缓存
